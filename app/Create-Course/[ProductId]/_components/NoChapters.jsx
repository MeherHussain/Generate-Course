...
const Chapters = ({ course }) => {
  // Ensure course and its nested properties are valid
  const chapters = course?.courseOutput?.chapters; // Fix the path to access chapters

  // Check if chapters is a valid array and get its length
  const chapterCount = Array.isArray(chapters) ? chapters.length : 0; // Default to 0 if not an array
      ...
      <div className="card">
        <div className="c-txt">
          ...
          <h2 className="font-bold">
            {chapterCount > 0 ? chapterCount : "No Chapters Available"}{" "}
            {/* Display exact number or fallback */}
          </h2>
        </div>
      </div>
    </StyledWrapper>
